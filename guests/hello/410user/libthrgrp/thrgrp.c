...
/**
 * @brief Destroys an initialized thrgroup
 *
 * Destroys the mutexes and other related stuff
 *
...
static void *thrgrp_bottom(void *in_data) {
    ...
    /* run the code we were asked to run */
    ret = func(arg);
...
int thrgrp_create(thrgrp_group_t *tg, void *(*func)(void *), void *arg) {
    ...
    /* setup a hunk of data
       ...
       and then as the memory to go on the zombie queue, this way we
       malloc only once */
    ...
    /* we don't return the tid, because you're not supposed to join on it
      must be joined with thrgrp_join(), not thr_join() */
    return 0;
}

/** @brief joins on any thread in the group which exits
 *
 ...
 * the thread we join on (i.e., what's returned from that thread's first function
